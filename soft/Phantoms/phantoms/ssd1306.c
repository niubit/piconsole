...
void spi_write_block(uint8_t *data, uint16_t count) {
    // Convenience function to write 'count' bytes to the matrix
    uint16_t r = spi_write_blocking(SPI_PORT, data, count);
    if (r != count) printf("(SPI err: %i", r);
}
...
void ssd1306_init(uint8_t pixel_width, uint8_t pixel_height) {
    ...
    spi_tx_buffer_size = oled_buffer_size + 1;

    // Initialise the SPI TX buffer
...
void ssd1306_write(bool is_command, uint8_t *data, uint16_t data_size) {
    // Write a command or data to the display via SPI
    gpio_put(PIN_DC, !is_command);

    // Copy the input data to index 1 of the TX array
    if (data_size <= spi_tx_buffer_size) {
        memcpy(&spi_tx_buffer[0], data, data_size);
    }
    gpio_put(PIN_CS, 0); // Indicate beginning of communication
    // Write out the TX array
    spi_write_block(spi_tx_buffer, data_size);
    gpio_put(PIN_CS, 1); // Indicate end of communication
}
