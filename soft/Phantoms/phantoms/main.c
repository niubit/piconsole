...
void setup() {
    ...
    gpio_init(PIN_CS); // Initialise CS Pin
    gpio_set_dir(PIN_CS, GPIO_OUT); // Set CS as output
    gpio_put(PIN_CS, 1); // Set CS High to indicate no currect SPI communication

    // Configure DC
    gpio_init(PIN_DC); // Initialise DC Pin
    gpio_set_dir(PIN_DC, GPIO_OUT); // Set DC as output
...
void game_loop() {
    ...
    do {
        ...
        if (!gpio_get(PIN_RIGHT)) x = LOWER_LIMIT-1;
        else if (!gpio_get(PIN_LEFT)) x = UPPER_LIMIT+1;
        else x = 2048;
        if (!gpio_get(PIN_DOWN)) y = LOWER_LIMIT-1;
        else if (!gpio_get(PIN_UP)) y = UPPER_LIMIT+1;
        else y = 2048;
