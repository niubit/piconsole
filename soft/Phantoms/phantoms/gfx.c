...
void animate_turn(bool is_left) {
    ...
    // NOTE We write directly to 'spi_tx_buffer' to save a 1KB memcpy()
    for (uint8_t n = 1 ; n < 128 ; n += 16) {
        ...
        for (uint8_t y = 0 ; y < 8 ; ++y) {
            ...
            if (is_left) {
                memcpy(&spi_tx_buffer[1 + (y << 7)], &side_buffer[(y << 7) + 128 - n], n);
                memcpy(&spi_tx_buffer[1+ (y << 7) + n], &oled_buffer[y << 7], 128 - n);
            } else {
                memcpy(&spi_tx_buffer[1 + (y << 7)], &oled_buffer[y * 128 + n], 128 - n);
                memcpy(&spi_tx_buffer[1 + (y << 7) + 127 - n], &side_buffer[y << 7], n);
            }
        }

        // Present the animation frame (takes approx. 20.5ms)
        spi_tx_buffer[0] = SSD1306_WRITE_TO_BUFFER;
        spi_write_block(&spi_tx_buffer[0], oled_buffer_size + 1);
    }
}
